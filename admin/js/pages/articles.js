$(document).ready(function () {
  if (domElemExists('#articleInsertPage')) {
    initTextEditor();
    initDatepicker();
    initCategoriesTree();
  }
...
$('.page-form').on('submit', function (event) {
  ...
  if (id == 0) {
    ...
    asyncInsertAction('/articles/insert', payload, AXIOS_HEADERS, '/administration/articles/:id/insert');

  } else {

    asyncUpdateAction('/articles/update', payload, AXIOS_HEADERS);
  }
});
  ...
  swal({
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        asyncDeleteAction('/articles/delete/' + id, AXIOS_HEADERS);
